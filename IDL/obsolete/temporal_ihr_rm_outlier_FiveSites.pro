PRO TEMPORAL_IHR_RM_OUTLIER_FIVESITES


;08/23/2017 - Jake Chung
;	This program is built based on temporal_ihr_BarrowCapeGrim_Aydin_FirnAir.pro
;	It is similar to that program but it filters out data that is more or less than 3 sigma
;	Check notes in notebook
;08/24/2017 - Jake Chung
;	More modifications to the program for removing the outliers. 
...
sigma_capegrim_ogi = stddev(detrend_capegrim_ogi) 
sigma_barrow_ogi = stddev(detrend_barrow_ogi)
sigma_barrow_uci = stddev(detrend_barrow_uci)
sigma_barrow_noaa = stddev(detrend_barrow_noaa)
sigma_samoa_ogi = stddev(detrend_samoa_ogi)
sigma_samoa_uci = stddev(detrend_samoa_uci)
sigma_samoa_noaa = stddev(detrend_samoa_noaa)

;removing the outliers
help, barrow_uci
outlier_idx_up = where(detrend_barrow_uci GT sigma_barrow_uci * 3)
outlier_idx_low = where(detrend_barrow_uci LT - sigma_barrow_uci * 3)
;concatnate the two arrays to perform RemoveRows function
outlier_idx = [outlier_idx_up , outlier_idx_low]
print, barrow_uci_time.year[outlier_idx]
barrow_uci = RemoveRows(barrow_uci, outlier_idx)
help, barrow_uci


help, barrow_ogi
outlier_idx_up = where(detrend_barrow_ogi GT sigma_barrow_ogi * 3)
outlier_idx_low = where(detrend_barrow_ogi LT - sigma_barrow_ogi * 3)
;concatnate the two arrays to perform RemoveRows function
outlier_idx = [outlier_idx_up , outlier_idx_low]
...
help, barrow_noaa
outlier_idx_up = where(detrend_barrow_noaa GT sigma_barrow_noaa * 3)
outlier_idx_low = where(detrend_barrow_noaa LT - sigma_barrow_noaa * 3)
;concatnate the two arrays to perform RemoveRows function
outlier_idx = [outlier_idx_up , outlier_idx_low]
...
help, samoa_uci
outlier_idx_up = where(detrend_samoa_uci GT sigma_samoa_uci * 3)
outlier_idx_low = where(detrend_samoa_uci LT - sigma_samoa_uci * 3)
;concatnate the two arrays to perform RemoveRows function
outlier_idx = [outlier_idx_up , outlier_idx_low]
...
help, samoa_noaa
outlier_idx_up = where(detrend_samoa_noaa GT sigma_samoa_noaa * 3)
outlier_idx_low = where(detrend_samoa_noaa LT - sigma_samoa_noaa * 3)
;concatnate the two arrays to perform RemoveRows function
outlier_idx = [outlier_idx_up , outlier_idx_low]
...
help, capegrim_ogi
outlier_idx_up = where(detrend_capegrim_ogi GT sigma_capegrim_ogi * 3)
outlier_idx_low = where(detrend_capegrim_ogi LT - sigma_capegrim_ogi * 3)
;concatnate the two arrays to perform RemoveRows function
outlier_idx = [outlier_idx_up , outlier_idx_low]
...
capegrim_ogi_time =  tau2yymmdd(capegrim_ogi[ 0 , * ], /GEOS1)
;PART 4:<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
;Part 4 is exactly the same as part 2, but since the data are different now
;so the outcome would also be different.
