PRO TEMPORAL_IHR_W_2SITES

;This program uses the American Samoa and Barrow AK sites from the 3 data networks
;to calculate the interhemispheric ratio.
...
infile = '/home/jakechung/IDL/myIDL/NOAA_Barrow_Samoa.dat/'
...
;for OGI 
ihr_ogi = mean( barrow_ogi[ 0 , * ] ) / mean( samoa_ogi[ 0 , * ] )
...
ihr_ogi_err = sqrt( (barrow_ogi_err/sou_ogi)^2 + (nor_ogi*samoa_ogi_err/(sou_ogi^2))^2 )
...
for i = 0, 33 do begin	;there are 34 years in the simulation, so from 0 to 33
	...
	temp_global_sou[i] = temp_annualSimArr[ i , 4 , 38 ] ;140 and 38 are the indecies of the Samoa station
endfor
...
cgplot, ogi_time, nor_ogi, /nodata, ytitle = 'Hemispheric Means (pptv)', xrange = [ 1982, 2015 ], charsize = 1.3, $
	symsize = 0.7, yrange = [ 100, 2200 ]
cgplot, ogi_time, nor_ogi, /overplot, err_ylow = barrow_ogi_err, err_yhigh = barrow_ogi_err, psym = 4, color = 'black'
cgplot, ogi_time, sou_ogi, /overplot, err_ylow = samoa_ogi_err, err_yhigh = samoa_ogi_err, psym = 14, color = 'black'
...
AL_Legend, ['Scenario A, chi-sq = 58.61', 'Scenario B, chi-sq = 311.9', 'Scenario C, chi-sq = 44.51', $
	'Scenario D, chi-sq = 46.97', 'Scenario E, chi-sq = 950.1', 'Scenario F, chi-sq = 19.83'], $	
	color = ['violet', 'forest green', 'blue', 'blue', 'red', 'red'], $
	psym = [ -3, -3, -3, -3, -3, -3], linestyle = [ 0, 0, 2, 0, 2, 0 ], box=0, $
